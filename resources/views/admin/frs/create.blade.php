...
@section('content')
    <h2>Tambah Data FRS</h2>
    <p class="text-muted">Isi data lengkap FRS yang ingin ditambahkan</p>

    <div class="card p-4">
        <form action="{{ route('admin.frs.store') }}" method="POST">
            @csrf
            <div class="row">
                ...
                <div class="mb-3 col-md-4">
                    ...
                    <select name="paket_frs_id" id="paket_frs_id" class="form-select" required>
                        @foreach ($paketFrsList as $paket)
                            <option value="{{ $paket->id }}" {{ old('paket_frs_id') == $paket->id ? 'selected' : '' }}>
                                {{ $paket->nama_paket }}
                            </option>
                        @endforeach
                    </select>

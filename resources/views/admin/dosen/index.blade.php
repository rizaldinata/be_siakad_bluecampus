@extends('layouts.admin')

@section('content')
    <h2>Data Dosen</h2>
    <p class="text-muted">Daftar semua Dosen</p>

    <div class="card p-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="mb-0">Tabel Dosen</h5>
            <a href="{{ route('admin.dosen.create') }}" class="btn btn-primary">
                <i class="bi bi-plus-lg me-1"></i> Tambah Dosen
            </a>
        </div>

        <div class="table-responsive">
            <table class="table table-hover align-middle">
                <thead class="table-light">
                    <tr>
                        <th>#</th>
                        <th>NIP</th>
                        <th>Nama</th>
                        <th>Email</th>
                        <th>No Telepon</th>
                        <th>Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($dosen as $index => $dsn)
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>{{ $dsn->nip }}</td>
                            <td>{{ $dsn->nama_lengkap }}</td>
                            <td>{{ $dsn->user->email }}</td>
                            <td>{{ $dsn->no_telp }}</td>
                            <td>
                                {{-- Lihat Detail --}}
                                <a href="{{ route('admin.dosen.show', $dsn->id) }}" class="btn btn-sm btn-info me-1">
                                    <i class="bi bi-eye-fill"></i>
                                </a>

                                {{-- Edit --}}
                                <a href="{{ route('admin.dosen.edit', $dsn->id) }}" class="btn btn-sm btn-warning me-1">
                                    <i class="bi bi-pencil-fill"></i>
                                </a>

                                {{-- Hapus --}}
                                <form action="{{ route('admin.dosen.destroy', $dsn->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn btn-sm btn-danger"
                                        onclick="return confirm('Yakin ingin menghapus?')">
                                        <i class="bi bi-trash-fill"></i>
                                    </button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="text-center text-muted">Belum ada data mahasiswa.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>
@endsection

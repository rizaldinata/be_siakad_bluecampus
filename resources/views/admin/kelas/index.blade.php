@extends('layouts.admin')

@section('content')
    <h2>Data Kelas</h2>
    <p class="text-muted">Daftar semua Kelas</p>

    <div class="card p-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="mb-0">Tabel Kelas</h5>
            <a href="{{ route('admin.kelas.create') }}" class="btn btn-primary">
                <i class="bi bi-plus-lg me-1"></i> Tambah Kelas
            </a>
        </div>

        <div class="table-responsive">
            <table class="table table-hover align-middle">
                <thead class="table-light">
                    <tr>
                        <th>#</th>
                        <th>Nama</th>
                        <th>Program Studi</th>
                        <th>Paralel</th>
                        <th>Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($kelas as $index => $kls)
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>{{ $kls->nama_kelas }}</td>
                            <td>{{ $kls->program_studi }}</td>
                            <td>{{ $kls->parallel_kelas }}</td>
                            <td>
                                {{-- Lihat Detail --}}
                                <a href="{{ route('admin.kelas.show', $kls->id) }}" class="btn btn-sm btn-info me-1">
                                    <i class="bi bi-eye-fill"></i>
                                </a>

                                {{-- Edit --}}
                                <a href="{{ route('admin.kelas.edit', $kls->id) }}" class="btn btn-sm btn-warning me-1">
                                    <i class="bi bi-pencil-fill"></i>
                                </a>

                                {{-- Hapus --}}
                                <form action="{{ route('admin.kelas.destroy', $kls->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn btn-sm btn-danger"
                                        onclick="return confirm('Yakin ingin menghapus?')">
                                        <i class="bi bi-trash-fill"></i>
                                    </button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="text-center text-muted">Belum ada data Kelas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>
@endsection
